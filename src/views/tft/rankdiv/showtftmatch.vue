...
import {gettftmatchlist} from '@/script/func'
import Showtftmacths from './showtftmatchs.vue'
...
const getmorematchs = async () => {
    if(props.match !== null && props.match !== undefined){
        const message = props.part1 + '/' +  props.ppuid
            if(!update.value){
                const temp = await gettftmatchlist(message,20);
                ...
                const temp = await gettftmatchlist(message,now.value.length);
                if(temp !== null && temp !== undefined){
    ...
    <div class="lolmatch-mid1" v-if="(props.match !== null) && (props.match !== undefined) && !update">
       <!--
        ...
        <li v-for="(matchid, index) in props.match" :key="index">
            <Showtftmacths :region=props.part1 :matchid=matchid :index=index :puuid = props.ppuid />
        </li>
        ...
        测试用
         -->
        <li>
            <Showtftmacths :region=props.part1 :matchid=props.match[0] :index=0 :puuid = props.ppuid />
        </li>
